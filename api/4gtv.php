...
if ($data === null && json_last_error() !== JSON_ERROR_NONE) {
    echo 'Error: JSON decode error: ' . json_last_error_msg() . "<br>";
    exit;
}
...
// 循环遍历 URL 列表，找到第一个不是 cds.cdn.hinet.net 的 URL
foreach ($urls as $url) {
...
if (strpos($finalUrl, 'https://4gtvfree-mozai.4gtv.tv') === 0) {
    ...
    // 错误处理：检查 get_playURL 是否返回空值
    if (empty($finalUrl)) {
        echo "Error: get_playURL returned empty value.";
        exit;
    }
    ...
    $urlParts = explode('?', $finalUrl);
    if (empty($urlParts)) {
        echo "Error: Invalid URL format.";
        exit;
    }
    $firstPart = $urlParts[0];
    $preArray = explode('/', $firstPart);
    ...
    $lastElement = $preArray[count($preArray) - 1];
    $midArray = explode('-', $lastElement);
